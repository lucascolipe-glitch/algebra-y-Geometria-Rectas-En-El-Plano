...
const steppers = {
  tp2b: {
    ...
    intro: String.raw`Hallar la recta que pasa por \(A=(-1,-2)\) y es paralela a \(L_1:y-3x-9=0\).`,
    complete: 'formas',
    steps: [
      String.raw`Reescribimos la recta dada: \(y=3x+9\). Su pendiente es \(m=3\).`,
      String.raw`Una recta paralela debe conservar la misma pendiente. Usamos la forma punto–pendiente: \[y-(-2)=3\bigl(x-(-1)\bigr).\]`,
      String.raw`Desarrollamos: \[y+2=3x+3\quad\Longrightarrow\quad \boxed{y=3x+1}.\]`,
      String.raw`En forma implícita: \[\boxed{-3x+y-1=0}.\] El punto \((-1,-2)\) verifica la ecuación y la pendiente sigue siendo \(3\).`
    ]
  },
  tp5: {
    tag: 'TP 5 · ejercicio 5',
    title: 'Paralelas, perpendiculares y coincidentes',
    intro: String.raw`\(L_1:3x+hy+4=0\) y \(L_2:2x-4y+k=0\).`,
    complete: 'posiciones',
    steps: [
      String.raw`Los vectores normales son \(\vec n_1=(3,h)\) y \(\vec n_2=(2,-4)\).`,
      String.raw`<strong>Paralelas.</strong> Los normales deben ser proporcionales: \(\frac32=\frac{h}{-4}\), de donde \(h=-6\). Para que no coincidan, debe cumplirse \(k\neq\frac83\).`,
      String.raw`<strong>Perpendiculares.</strong> Los normales también son perpendiculares: \[(3,h)\cdot(2,-4)=0\quad\Longrightarrow\quad 6-4h=0,\] por lo tanto \(h=\frac32\). El valor de \(k\) es libre.`,
      String.raw`<strong>Coincidentes.</strong> Todos los coeficientes deben ser proporcionales: \[h=-6,\qquad \frac{4}{k}=\frac32,\] de modo que \(\boxed{h=-6,\;k=\frac83}.\]`
    ]
  },
  tp7: {
    tag: 'TP 5 · ejercicio 7',
    title: 'Mediatriz y puntos equidistantes',
    intro: String.raw`Sean \(A=(-1,-2)\) y \(B=(3,6)\).`,
    complete: 'mediatriz',
    steps: [
      String.raw`El punto medio es \[M=\left(\frac{-1+3}{2},\frac{-2+6}{2}\right)=(1,2).\]`,
      String.raw`El vector \(\overrightarrow{AB}=(4,8)\) es normal a la mediatriz. Entonces: \[4(x-1)+8(y-2)=0.\]`,
      String.raw`Simplificando: \[\boxed{x+2y-5=0}.\]`,
      String.raw`Sobre el eje \(x\), imponemos \(y=0\) y obtenemos \(P=(5,0)\).`,
      String.raw`Con \(L:2x-y+1=0\), la intersección con la mediatriz es \(\left(\frac35,\frac{11}{5}\right)\). En cambio, \(2x+4y+4=0\) es paralela a la mediatriz y no aporta un punto equidistante.`
    ]
  },
  tp12c: {
    tag: 'TP 5 · ejercicio 12.c',
    title: 'Distancia entre dos rectas paralelas',
    intro: String.raw`\(L_1:\;x=-2+t,\;y=3-t\) y \(L_2:\;x=5+k,\;y=-2-k\).`,
    complete: 'distancias',
    steps: [
      String.raw`Ambas rectas tienen director \((1,-1)\), por lo que son paralelas.`,
      String.raw`Convertimos \(L_1\) a forma implícita: \[x+y-1=0.\]`,
      String.raw`Tomamos el punto \(P=(5,-2)\in L_2\). Entonces: \[d(L_1,L_2)=d(P,L_1)=\frac{|5-2-1|}{\sqrt{1^2+1^2}}.\]`,
      String.raw`Finalmente: \[\boxed{d(L_1,L_2)=\frac{2}{\sqrt2}=\sqrt2}.\]`
    ]
  },
  tp14: {
    tag: 'TP 5 · ejercicio 14',
    title: 'Punto de una recta más cercano a un punto exterior',
    intro: String.raw`\(A=(1,-2)\) y \(L:(x,y)=\lambda(3,4)\).`,
    complete: 'distancias',
    steps: [
      String.raw`Un punto genérico de \(L\) es \(P(\lambda)=(3\lambda,4\lambda)\). El vector \(\overrightarrow{PA}=A-P(\lambda)\) debe ser perpendicular al director \((3,4)\).`,
      String.raw`Imponemos: \[(1-3\lambda,-2-4\lambda)\cdot(3,4)=0.\]`,
      String.raw`Se obtiene \(-5-25\lambda=0\), por lo tanto \(\lambda=-\frac15\) y \[\boxed{P=(-\frac35,-\frac45)}.\]`,
      String.raw`La distancia mínima es \[d(A,L)=\|A-P\|=\sqrt{(\frac85)^2+(-\frac65)^2}=\boxed{2}.\]`,
      String.raw`La paralela a \(L\) que pasa por \(A\) tiene ecuación \(4x-3y-10=0\) y está a distancia \(2\) de \(L:4x-3y=0\). Por eso el inciso de distancia \(4\) no es posible, mientras que el de distancia \(2\) sí.`
    ]
  },
  tp17: {
    tag: 'TP 5 · ejercicios 17 y 18',
    title: 'Familias o haces de rectas',
    intro: String.raw`El parámetro permite describir infinitas rectas que comparten una propiedad.`,
    complete: 'practica',
    steps: [
      String.raw`Paralelas a \(3y-x-3=0\): \[\boxed{y=\frac13x+k},\qquad k\in\mathbb R.\]`,
      String.raw`Perpendiculares a \(3x+2y-7=0\): la pendiente pedida es \(\frac23\), por ejemplo \[\boxed{y=\frac23x+k}.\]`,
      String.raw`Rectas que pasan por \(P=(4,-2)\): \[\boxed{y+2=k(x-4)}.\] Esta expresión no incluye la vertical \(x=4\), que debe agregarse aparte.`,
      String.raw`La familia \(-3x+3y+k=0\) conserva el vector normal y está formada por paralelas. La familia \(y=kx-8\) conserva el punto \((0,-8)\).`,
      String.raw`En \(\frac{x}{-2}+\frac{y}{k}=1\), todas las rectas pasan por el corte fijo \((-2,0)\).`
    ]
  }
};

const quizBank = [
  {q:String.raw`Una recta pasa por \(P_0=(2,-1)\) y tiene director \((3,4)\). ¿Cuál es su forma vectorial?`, options:[String.raw`\((x,y)=(2,-1)+\lambda(3,4)\)`,String.raw`\((x,y)=(3,4)+\lambda(2,-1)\)`,String.raw`\(3x+4y=0\)`,String.raw`\(y=4x+3\)`], answer:0, exp:'La forma vectorial usa un punto de la recta más un múltiplo del vector director.'},
  ...
  {q:String.raw`Dos rectas de pendientes \(2\) y \(-\tfrac12\) son…`, options:['paralelas','coincidentes','perpendiculares','verticales'], answer:2, exp:String.raw`El producto de pendientes es \(-1\).`},
  ...
  {q:String.raw`La mediatriz de \(AB\) pasa por…`, options:['el origen',String.raw`el punto medio de \(AB\)`,'el extremo A solamente','cualquier punto del eje x'], answer:1, exp:'Además de pasar por el punto medio, es perpendicular al segmento.'},
  {q:String.raw`Si \(A=B\), ¿qué ocurre con la recta determinada por esos dos puntos?`, options:['es vertical','es horizontal','no queda determinada de manera única','pasa por el origen'], answer:2, exp:'Dos puntos coincidentes no aportan una dirección.'},
  {q:String.raw`Para el área con base \(AB\), la altura es…`, options:[String.raw`\(d(A,B)\)`,String.raw`\(d(C,L_{AB})\)`,String.raw`\(d(A,C)\)`,'la pendiente de AB'], answer:1, exp:'La altura es la distancia perpendicular del vértice opuesto a la recta que contiene la base.'},
  {q:'Una recta vertical se escribe naturalmente como…', options:[String.raw`\(y=mx+n\)`,String.raw`\(x=x_0\)`,String.raw`\(y=y_0\)`,String.raw`\(m=0\)`], answer:1, exp:'La pendiente de una vertical no existe, pero su coordenada x es constante.'},
  ...
  {q:String.raw`La familia \(y=kx-8\) tiene en común…`, options:['la pendiente',String.raw`el punto \((0,-8)\)`,'el vector normal','ser horizontal'], answer:1, exp:'Al poner x=0 siempre se obtiene y=-8.'},
  {q:String.raw`La pendiente asociada al director \((4,-2)\) es…`, options:[String.raw`\(-2\)`,String.raw`\(-\tfrac12\)`,String.raw`\(2\)`,String.raw`\(\tfrac12\)`], answer:1, exp:'m=u_y/u_x=-2/4=-1/2.'},
  ...
  {q:String.raw`La ecuación segmentaria \(\frac{x}{A}+\frac{y}{B}=1\) muestra directamente…`, options:['el normal','los cortes con los ejes','el punto medio','el ángulo entre rectas'], answer:1, exp:'Los puntos de corte son (A,0) y (0,B).'}
];
...
const triangleSteps=[
  String.raw`Tomamos como base \(\overline{AB}\): \[b=d(A,B)=\sqrt{(4-1)^2+(-1-2)^2}=\sqrt{18}=3\sqrt2.\]`,
  String.raw`La recta que pasa por \(A=(1,2)\) y \(B=(4,-1)\) tiene dirección \((3,-3)\) y ecuación: \[\boxed{L_{AB}:x+y-3=0}.\]`,
  String.raw`La altura es la distancia de \(C=(5,2)\) a \(L_{AB}\): \[h=\frac{|5+2-3|}{\sqrt{1^2+1^2}}=\frac4{\sqrt2}=2\sqrt2.\]`,
  String.raw`Finalmente: \[\mathcal A=\frac{b\,h}{2}=\frac{(3\sqrt2)(2\sqrt2)}2=\boxed{6\text{ unidades cuadradas}}.\]`
];
...
function updateTriangleStep(){ const content=$('triangleStepContent'),button=$('triangleNextStep'); if(triStep===0){content.innerHTML='<p class="step-placeholder">La resolución aparecerá paso a paso.</p>';button.textContent='Mostrar paso 1';}else{content.innerHTML=triangleSteps.slice(0,triStep).map((step,i)=>`<article class="step-item visible"><span class="step-number">${i+1}</span><div>${step}</div></article>`).join('');button.textContent=triStep<triangleSteps.length?`Mostrar paso ${triStep+1}`:'Reiniciar resolución';}typeset(content);}
...
function openStepper(key){
  ...
  const render=()=>{const box=$('guidedStep');if(index===0)box.innerHTML='<p class="step-placeholder">Intentá plantear una estrategia antes de mostrar el primer paso.</p>';else box.innerHTML=data.steps.slice(0,index).map((step,i)=>`<article class="step-item visible"><span class="step-number">${i+1}</span><div>${step}</div></article>`).join('');$('nextGuidedStep').textContent=index<data.steps.length?`Mostrar paso ${index+1}`:'Resolución completa ✓';$('nextGuidedStep').disabled=index>=data.steps.length;typeset(mount);};
